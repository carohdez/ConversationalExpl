...
#wooz_sentences = pd.read_csv('C:\Python\ConversationalRS\data\WoOzSentences.csv')#to check accuracy on WoOz dataset
...
def get_intention():
    ...
    try:
        ...
        if 'aspect' in response.json():
            detail = response.json().get("aspect")
        else:
            detail = "not_found"
        ...
        print(sentence + str(entities_new) + str(intention))
    ...
    if len(intention) == 0:
       abort(404)
    return jsonify({'intention': intention[0], 'entities_new': entities_new})
